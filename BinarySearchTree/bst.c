...
Name:Kyle Livermore
Date: 7 Nov 2015
struct Implementation of the binary search tree data structure.
 */
#include <stdlib.h>
#include <stdio.h>
#include <assert.h>
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	while (1)
	{
		if (current == NULL)
		{
			return 0; //Traversed tot the end of the tree with out finding the value
		}
		else if (compare(current->val, val) == 0)
		{
			return 1;
		}
		else if (compare(current->val, val) > 0)
	...
	return 0; // unecessary but added incase the code in the while true loop is changed at a later point in time
}
